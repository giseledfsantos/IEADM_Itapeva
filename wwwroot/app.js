...
function renderMembersScreen(schema, table) {
  ...
  btnCriar.onclick = async () => {
    ...
    try {
      ...
      txt(out, res)
    } catch (e) { txt(out, String(e.message || e)) }
  }

  btnSalvar.onclick = async () => {
    out.textContent = ''
    if (!idInput.value.trim()) { txt(out, 'Selecione um membro ou informe ID'); return }
    try { txt(out, await apiUpdate(table.name, table.pk, idInput.value.trim(), collectPayload(form))) } catch (e) { txt(out, String(e.message || e)) }
  }

  btnExcluir.onclick = async () => {
    out.textContent = ''
    if (!idInput.value.trim()) { txt(out, 'Selecione um membro ou informe ID'); return }
    try { await apiDelete(table.name, table.pk, idInput.value.trim()); txt(out, 'Excluído'); btnLimpar.click() } catch (e) { txt(out, String(e.message || e)) }
  }
